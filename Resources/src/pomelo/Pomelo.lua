...
local Pomelo = class("Pomelo",Emitter)
...
function Pomelo:_setTimeout(fn,delay)
    if not self._schedulerNode then
        self._schedulerNode = cc.Node:create()
        performWithDelay(self._schedulerNode,fn,delay)
    end
end

function Pomelo:_clearTimeout(fn)
    if fn and fn ~= 0 then
        if self._schedulerNode then
